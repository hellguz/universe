// Simulation fragment shader - N-body gravity physics
...
void main() {
    ...
    // Note: We're writing to two separate render targets in the FBO component
    // For now, this shader will be run twice with different outputs

    // Position output
    gl_FragColor = vec4(position, mass);

    // Velocity output (will be handled by dual render targets in FBO)
